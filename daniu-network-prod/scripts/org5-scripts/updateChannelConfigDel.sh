...
set -x
# jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"Org5MSP":.[1]}}}}}' config.json ./organizations/peerOrganizations/org5.niuinfo.com/org5.json > modified_config.json
jq 'del(.channel_group.groups.Application.groups.Org5MSP)' config.json > modified_config.json
{ set +x; } 2>/dev/null
...
# Note: This is different from adding org3 and org4 which only need sign by org1 and org2
infoln "签名 Signing config transaction by org3"
signConfigtxAsPeerOrg 3 org5_update_in_envelope.pb

infoln "签名 Signing config transaction by org4"
...
setGlobals 2
set -x
peer channel update -f org5_update_in_envelope.pb -c ${CHANNEL_NAME} -o orderer.niuinfo.com:7050 --ordererTLSHostnameOverride orderer.niuinfo.com --tls --cafile ${ORDERER_CA}
{ set +x; } 2>/dev/null
