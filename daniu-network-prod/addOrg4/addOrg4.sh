...
function generateOrg4Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org4MSP > ../organizations/peerOrganizations/org4.niuinfo.com/org4.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function addOrg4 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org4.niuinfo.com" ]; then
    generateOrg4
  ...
  # Note: 原来的逻辑是首先复制organizations/peerOrganizations/org4.niuinfo.com到第一台节点，然后到第一台节点cli上执行
  # Use the CLI container to create the configuration transaction needed to add
  # Org4 to the network
  infoln "Generating and submitting config tx to add Org4"
  docker exec cli4 ./scripts/org4-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to create config tx"
  fi

  infoln "Joining Org4 peers to network"
  docker exec cli4 ./scripts/org4-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
